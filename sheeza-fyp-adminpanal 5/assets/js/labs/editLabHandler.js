...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch(`${url}/api/lab/get/${id}`);
        const lab = await response.json();
        document.querySelector('#labName').value = lab.labName
                                                   .split(' ') 
                                                   .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                                                   .join(' ');
        document.querySelector('#deptName').value = lab.deptName
                                                    .split(' ') 
                                                    .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                                                    .join(' ');
    } catch (error) {
...
document.querySelector('.submit-btn').addEventListener('click', async (event) => {
    ...
    try {
        const response = await fetch(`${url}/api/lab/update/${id}`, {
            method: 'PUT',
            headers: {
