...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    try {
        const response = await fetch(`${url}/api/pc/get/${id}`);
        const data = await response.json();
        document.querySelector('#pcName').value = data.pcName
                                                .split(' ') 
                                                .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                                                .join(' ');
        document.querySelector('#labName').value = data.labName
                                                    .split(' ') 
                                                    .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                                                    .join(' ');
    } catch (error) {
...
document.querySelector('.submit-btn').addEventListener('click', async(event) => {
    ...
    try {
        const response = await fetch(`${url}/api/pc/update/${id}`, {
            method: 'PUT',
            headers: {
